...
class WeightedAStarAgent(Agent):
    ...
    def h_msap(self,  state: Tuple) -> int:
        ...
        d1_dist = abs(self.env.to_row_col(self.env.d1)[0] - self.env.to_row_col(state)[0]) + abs(self.env.to_row_col(self.env.d1)[1] - self.env.to_row_col(state)[1])
                
        if d1_dist < min_dist:
            min_dist = d1_dist
        
        d2_dist = abs(self.env.to_row_col(self.env.d2)[0] - self.env.to_row_col(state)[0]) + abs(self.env.to_row_col(self.env.d2)[1] - self.env.to_row_col(state)[1])
        if d2_dist < min_dist:
            min_dist = d2_dist
